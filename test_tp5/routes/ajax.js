 ...
 */
 router.post('/sauvegarderExamen', function (req, res) {
    ...
    var results = new ExamenSchema({
        ...
        toComplete: req.body.toComplete
    });
...
router.get('/examenToContinue',function(req,res){

    var condition =  { toComplete: { $in: ['true'] } };
    ExamenSchema.findOne(condition,function(err,result){
        if(!err){
            console.log(result)
            res.json(result)
        }
